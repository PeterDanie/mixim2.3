...
void NodeFactory::handleMessage(cMessage * msg)
{
    ...
    ev << "Current node number = " << node_count << endl;
    node_count_vector.record(node_count);
    recordScalar("#nodes", node_count);
}
...
void NodeFactory::createNode(CreateEv * event)
{
    ...
    if (mobility != 0)
    {
        ...
        if (mobility->hasPar("initialX"))
            mobility->par("initialX") = event->getX();
        if (mobility->hasPar("initialY"))
            mobility->par("initialY") = event->getY();
        if (mobility->hasPar("initialZ"))
            mobility->par("initialZ") = event->getZ();
    }
    ...
    if (_pendingWaypointsLists[event->getNodeID()].size() != 0)
    {
        ...
        cModuleType *traceMobilityType = 
                cModuleType::get("inet.mobility.models.TraceMobility");
        if (mobilityType != traceMobilityType)
            throw
            cRuntimeError
            ("Node does not have mobility module of type inet.mobility.models.TraceMobility");
